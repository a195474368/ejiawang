...
{   
    ...
    protected $_cols = 'tracking_id,look_id,content,create_ip,dateline,reply,reply_time,pd';
    ...
    //回复消息,关联判断查询字段
        public function getNoReplyCountByTendersIdss($ids,$tdp){
        if(empty($ids)) return array();
        foreach($ids as $k=>$v){
            $ids[$k] = (int) $v;
        }
        $idstr = join(',',$ids);
        $sql = " SELECT a.tenders_id,count(b.tracking_id) as num from ".$this->table($this->_table)." b JOIN  ".$this->table('tenders_look')." a  ON(b.look_id =a.look_id) where a.tenders_id in({$idstr}) AND b.reply is not null AND b.pd = 0 AND a.company_id = ".$tdp."  group by  a.tenders_id  ";
        $items = array();
        if($rs = $this->db->Execute($sql)){
            while($row = $rs->fetch()){
                    $items[$row['tenders_id']] = $row['num'];
