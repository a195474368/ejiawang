...
class Ctl_Ucenter_Tenders extends Ctl_Ucenter {
    ...
    public function  index($page = 1){
        ...
        //关联查询(从判断表查询) 获取判断字段   
        $this->pagedata['pd'] = K::M('tenders/tracking')->items(array('look_id'=>$look_id)); 
         foreach($this->pagedata['pd'] as $vs){
                if( $vs['pd'] == 0 ){
                    $look_ids[$vs['look_id']] = $vs['look_id']; 
                }                      
            }
        //判断是否有值
        if( $look_ids !=null ){
            //关联查询(从中间表查询) 获取判断字段   
            $this->pagedata['pds'] = K::M('tenders/look')->items(array('look_id'=>$look_ids)); 
             foreach($this->pagedata['pds'] as $vs){
                    $tenders_ides[$vs['tenders_id']] = $vs['tenders_id'];
            }
            //分配搜索字段
            $this->pagedata['tenders_ides'] = $tenders_ides;
        }



        $this->tmpl = 'ucenter/tenders/index.html';  
    }
    ...
    public function tracking($look_id = null){
        ...
        elseif(!$look = K::M('tenders/look')->detail($look_id)){
            ...
            //插入判断字段   
            $this->pagedata['pd'] =K::M('tenders/tracking')->update($pid,array('pd'=>'1'));


            $this->tmpl = 'ucenter/tenders/tracking.html';
        }      
    }
    ...
    public function in($page = 1){
        ...
        $this->pagedata['items'] = $items;
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'ucenter/tenders/in.html';
    }
    ...
    public function view($id = null){
         ...
         if(!($id = (int)$id) && !($id = (int)$this->GP('id'))){
            ...
            $this->tmpl = 'ucenter/tenders/view.html';
        }
    }
