...
class Ctl_Home extends Ctl {
    ...
    public function items()
    {
            ...
            // $this->pagedata['items_name']=trim($_POST['s']);
     
    
             $home_name=trim($_GET['s']);
        $url = array();
        ...
        $this->pagedata['items_name']=$home_name;
        $this->pagedata['pager'] = $pager;
    ...
    public function survey()
    {
        ...
        if (!$data = $this->GP('data')) {
            ...
            if( $cityName=="武汉" ){
                $msg="亲爱的".$name."先生／女士您好，您于".date("m",time())."月".date("d",time())."日".date("H",time())."时".date("i",time())."分提交小区团装或免费看样板房服务申请，稍后客服027-59103095将会给您致电，请保持电话畅通，如需加急请致4008338583";
            }else{
                $msg="亲爱的".$name."先生／女士您好，您于".date("m",time())."月".date("d",time())."日".date("H",time())."时".date("i",time())."分提交小区团装或免费看样板房服务申请，十分钟内我们客服将会给您致电，请保持电话畅通，如需加急请致电4008338583";
            }
                
            $urlruten=new Ctl_Msg($msg ,$data['tel']);
            $urlruten->index();  
