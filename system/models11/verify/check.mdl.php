...
class Mdl_Verify_Check
{
	...
	{
		return strlen($phone)==11 && preg_match("/^(1[3,5,8,7]{1}[\d]{9})|(((400)-?(\d{3})-?(\d{4}))|^((\d{7,8})|(\d{4}|\d{3})-?(\d{7,8})|(\d{4}|\d{3})-(\d{3,7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$)$/", $phone);
		//return preg_match("/^(0?(([1-9]\d)|([3-9]\d{2}))-?)?\d{7,8}$/", $phone);
	}

	public static function mobile($mobile)
	{
	return strlen($mobile)==11 && preg_match("/^1[3-8]\d{9}$/", $mobile);
	}
