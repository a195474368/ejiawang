...
class Ctl_Ucenter_DesignerCase extends Ctl_Ucenter {
    ...
    public function index($page = 1) {
        ...
        $this->tmpl = 'ucenter/designerCase/index.html';
    }
    ...
    public function pic($case_id = null,$page=1){
        ...
        if (!($case_id = (int) $case_id) && !($case_id = (int)$this->GP('case_id'))) {
            ...
            if($items = K::M('case/photo')->items_by_case($case_id, $page, $limit, $count)){
                $this->pagedata['items'] = $items;
                $pager['count'] = $count;
                $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink("ucenter/designerCase:pic", array($case_id,'{page}'),array(),true));
            }
            $this->pagedata['pager'] = $pager;
            $this->pagedata['case']  = $detail;
            $this->tmpl = 'ucenter/designerCase/pic.html';
        }
    }
    ...
    public function edit($case_id = null) {
        ...
        if (!($case_id = (int) $case_id) && !($case_id = (int)$this->GP('case_id'))) {
            ...
            $this->pagedata['detail'] = $detail;
            $this->tmpl = 'ucenter/designerCase/edit.html';
        }
    }
        ...
        elseif (($audit = K::M('system/audit')->audit('case', $this->MEMBER)) == -1) {
            ...
            if (!$data = $this->check_fields($data, $this->_case_create_allower_fields)) {
                ...
                if ($case_id = K::M('case/case')->create($data)) {
                    ...
                    $this->err->add('添加内容成功');
                    $this->err->set_data('forward', $this->mklink('ucenter/designerCase:pic', array($case_id), array(), true));
                }
            }
        } else {
             $this->tmpl = 'ucenter/designerCase/create.html';
        }
    }
